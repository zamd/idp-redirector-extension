...
describe("#idp-redirector", () => {
  ...
  describe("PUT /api", () => {
      ...
      request(app)
        ...
        .end((err, res) => {
          ...
          expect(res.body)
            ...
            .deep
            .equal(whiteListData);
          expect(storage.data.whiteList)
            .to
            .deep
            .equal(whiteListData);
          expect(storage.data.hostToPattern)
            .to
            .deep
            .equal(expectedHostToPattern);
          done();
        });
    });
    ...
    it('fails with wildcard in hostname',
    ...
    it('fails with wildcard somewhere in the middle of the pattern',
      whiteListFailureTest([{
          clientName: 'client name',
    ...
    it("fails with invalid key", whiteListFailureTest([{
        clientName: 'the client',
    ...
    it("fails with empty patterns", whiteListFailureTest([{
        clientName: 'the client',
        patterns: []
      }],
    ...
    it("fails with empty request", (done) => {
      request(app)
        ...
        .end((err, res) => {
          ...
          expect(res.body.error_description)
            .to
            .equal('ValidationError: "value" must be an array');
          done();
        });
    });
  });
